...
#if defined(CONFIG_ENV_PARTITION)
        int old_part;
#endif
...
#if defined(CONFIG_ENV_PARTITION)
        old_part = mmc->part_num;
        if (old_part != CONFIG_ENV_PARTITION &&
            mmc_switch_partition(mmc, CONFIG_ENV_PARTITION, 0) < 0) {
                printf("%s ERROR: couldn't switch to partition %d\n", __FUNCTION__,
                       CONFIG_ENV_PARTITION);
                return 1;
        }
        mmc->part_num = CONFIG_ENV_PARTITION;
#endif
...
#if defined(CONFIG_ENV_PARTITION)
        if (old_part != CONFIG_ENV_PARTITION &&
            mmc_switch_partition(mmc, old_part, 0) < 0) {
                printf("%s ERROR: couldn't switch to partition %d\n", __FUNCTION__,
                       old_part);
                return 1;
        }
        mmc->part_num = old_part;
#endif
