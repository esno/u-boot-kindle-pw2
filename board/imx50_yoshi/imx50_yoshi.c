...
{
#if defined(CONFIG_CMD_IDME)
    if (idme_check_update()) idme_clear_update();
...
int board_late_init(void)
{
	...
	/* drive CHRG_DET_INT_B low on Icewine to avoid trickle charge */
	if (BOARD_IS_ICEWINE(board->id) || BOARD_IS_ICEWINE_WFO(board->id)) {
		mxc_request_iomux(MX50_PIN_EIM_RDY, IOMUX_CONFIG_GPIO);
		mxc_iomux_set_pad(MX50_PIN_EIM_RDY,
				PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PULL |
				PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_DRV_HIGH|
				PAD_CTL_100K_PD);

		mx50_gpio_direction(IOMUX_TO_GPIO(MX50_PIN_EIM_RDY), MX50_GPIO_DIRECTION_OUT);
		mx50_gpio_set(IOMUX_TO_GPIO(MX50_PIN_EIM_RDY), 0);
	}
