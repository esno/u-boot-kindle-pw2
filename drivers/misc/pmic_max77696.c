...
#define UIC_MANCTRL_ISET_0P1A               0x9		/* MANSET=b1 ; ISET=b001 (0.1A - SDP100) */
#define UIC_MANCTRL_ISET_0P4A               0xA		/* MANSET=b1 ; ISET=b010 (0.4A - UART) */
#define UIC_MANCTRL_ISET_0P5A               0xB		/* MANSET=b1 ; ISET=b011 (0.5A - SDP500) */

#define MID_VCOMR  -2499     //-2499mV
#define MAX_VCOMR  0         //0mV
#define MIN_VCOMR  -5000     //-5000mV
#define MAX_VDDH  29570      //29579 mV
#define MIN_VDDH  15000      // 15000mV
#define EPDVCOM_STEP 98
#define EPDVDDH_STEP 470
...
void low_battery_check_loop(void)
{
	...
	int retry;
        volatile int crit_batt_displayed = 0;
		...
		for(retry=10;
			...
			if( PMIC_CMD_PASS == pmic_fg_read_voltage(&batt_vol) ) {
				...
				break;
                        ...
                        if (!crit_batt_displayed) {
                                gd->flags |= GD_FLG_CRIT_BATT;
#ifdef CONFIG_SPLASH_COMPRESSED
                                drv_lcd_init_noconsole();
#endif
                                crit_batt_displayed = 1;
                                gd->flags &= ~GD_FLG_CRIT_BATT;
                        }
...
static void pmic_fl_init(void)
{
	...
	if (gd->flags & GD_FLG_QUICKBOOT) {
		...
		if (!board_is_rtc_wakeup(MAX_77696_RTC1) && !board_is_rtc_wakeup(MAX_77696_RTC2)
			...
			pmic_fl_set(DUET_FL_LEVEL12_MID);
		}
		puts("fl\n");
		gd->flags |= GD_FLG_QUICKBOOT;
	} else {
	...
	if (BOARD_IS_WHISKY_WFO(rev) || 
		...
		BOARD_IS_WOODY(rev)) {
		end_intensity = DUET_FL_LEVEL12_MID;
	}
...
static void setup_regulators(void)
{
	...
	if(cid < 0x20) {
        ...
        /*
         * RMAICEWINE-109 workaround applicable to all Wario devices.
         * BUCK_PWR_MD = 0b11 (Normal Operation).
         * That reduces the amplitude and duration of voltage droop on SW6_3V2.
         */
        pmic_write_reg(PM_GPIO_SADDR, BUCK_VOUTCNFG6_REG, 0x70);
...
int pmic_gpio1_direction_output(int val)
{
	int rval;

	if (val) 
		rval = pmic_write_reg(PM_GPIO_SADDR, GPIO_CNFG_GPIO1_REG, 0x9);	/* PUSH-PULL o/p = 1 */
	else
		rval = pmic_write_reg(PM_GPIO_SADDR, GPIO_CNFG_GPIO1_REG, 0x1);	/* PUSH-PULL o/p = 0 */
...
int pmic_init(void)
{
     ...
     */
    pmic_write_reg(USBIF_SADDR, UIC_MANCTRL_REG, UIC_MANCTRL_ISET_0P5A);

    /* Set FCC to 133mA for Whisky only considering PQSEL2A & PQSEL1A are grounded by HW for pre-qual phase
     */
    if (BOARD_IS_WHISKY_WAN(rev) || BOARD_IS_WHISKY_WFO(rev)) 
        pmic_charger_set_current(MAX77696_CHARGER_CURRENT_P133A);
...
/* Set PMIC LED to manual mode (green & amber LED) */
int pmic_led_manual_mode (void) 
{
    if(gpio_get_value(MX6_SODA_CHG_DET)) { 		/* USB charger disconnected */
        if (!pmic_write_reg(PM_GPIO_SADDR, LEDIND_STAT2FLASH_REG, 0x0F)) {
	        printf("PMIC write error LEDIND_STAT2FLASH_REG \n");
	        return PMIC_I2C_ERROR;
        }
    } else {									/* USB charger connected */
        if (!pmic_write_reg(PM_GPIO_SADDR, LEDIND_STAT2FLASH_REG, 0xF0)) {
	        printf("PMIC write error LEDIND_STAT2FLASH_REG \n");
	        return PMIC_I2C_ERROR;
        }
    }
    
    if (!pmic_wor_reg(PM_GPIO_SADDR, LEDIND_STAT2LEDSET_REG, LEDIND_REG_BITSET(STATLEDSET, LEDEN, 1), LEDIND_STATLEDSET_LEDEN_M)) {
	    printf("PMIC write error LEDIND_STAT2LEDSET_REG \n");
	    return PMIC_I2C_ERROR;
    }
	
    if (!pmic_write_reg(PM_GPIO_SADDR, LEDIND_STAT1FLASH_REG, 0x0F)) {
	    printf("PMIC write error LEDIND_STAT1FLASH_REG \n");
	    return PMIC_I2C_ERROR;
    }

    if (!pmic_wor_reg(PM_GPIO_SADDR, LEDIND_STAT1LEDSET_REG, LEDIND_REG_BITSET(STATLEDSET, LEDEN, 1), LEDIND_STATLEDSET_LEDEN_M)) {
	    printf("PMIC write error LEDIND_STAT1LEDSET_REG \n");
	    return PMIC_I2C_ERROR;
    }
...
int set_vddh_value(int vddh_val)
{
	int epdvddh = 0;
#if defined(CONFIG_QBOOT)
	if (! (gd->flags & GD_FLG_QUICKBOOT)) {
		printf(" set VDDH value:%dmV\n", vddh_val);
	}
#endif
	if(vddh_val < MIN_VDDH || vddh_val > MAX_VDDH)
	{
             printf("Valid VDDH range is 15000 to 29570mV\n");
             return PMIC_VNI_TEST_FAIL;
	}

	epdvddh = (int)((vddh_val - MIN_VDDH)/EPDVDDH_STEP);

	if(!pmic_write_reg(PM_GPIO_SADDR,VREG_EPDVDDH_REG,epdvddh))
	{
		   printf("PMIC write error (PM_GPIO_SADDR,VREG_EPDVDDH_REG,epdvddh) \n");
		   return PMIC_I2C_ERROR;
	}
	return PMIC_CMD_PASS;
}
...
int pmic_vddh_set(char* vddh_value) {
	int vddh_val;

	vddh_val = simple_strtol(vddh_value, NULL, 10);
	pmic_vddh_set_int(vddh_val);
}

int pmic_vddh_set_int(int vddh_val)
{
	int retry = PMIC_CHECK_POWER_GOOD_NUM_RETRY;
	if (!is_epdc_on())
	{
	...
	if (!is_epdc_on()) {
      ...
      /*
       sets VDDH value
      */
     if(set_vddh_value(vddh_val) != PMIC_CMD_PASS)
     {
	    pmic_enable_epdc(0);
		return PMIC_VNI_TEST_FAIL;
     }
