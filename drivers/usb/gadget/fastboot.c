...
extern const struct partition_info_t *get_partition_info_for_device(void);
/*
 * Serial number
 */
...
#ifdef CONFIG_WARIO            
            /*
             * For Wario we only use Boot Partition 1 for booting.
             * Boot Partition 2 is used as regular storage.
             */
            if (mmc_partition != 1)
                enable_boot = 0;
#endif
	    if (mmc_switch_partition(mmc, mmc_partition, enable_boot) < 0) {
