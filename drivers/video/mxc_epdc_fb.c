...
static inline int epdc_is_wb_active(void)
{
	u32 val = REG_RD(EPDC_BASE, EPDC_STATUS);
	int is_active = val & EPDC_STATUS_WB_BUSY ? TRUE : FALSE;
...
static void epdc_init_settings(void)
{
	...
	/* EPDC_FORMAT - 2bit TFT and 5bit Buf pixel format */
...
{
	int i;
	uint16_t * wb = panel_info.epdc_data.working_buf_addr;
	uint16_t * end = wb + (panel_info.vl_col * panel_info.vl_row);
	...
	for (i = 0; i < 40; i++) {
		if (!epdc_is_lut_active(0)) {
			...
			// Make the working buffer reflect that every pixel is
			// currently white.
			for (;wb < end; wb++) {
				*wb = 0x1e;
			}
...
void draw_splash_screen(int col, int row, int cols, int rows)
{
	int i;
	int lut_num = 0;

	/* Program EPDC update to process buffer */
	epdc_set_update_coord(col, row);
	epdc_set_update_dimensions(cols, rows);
	epdc_submit_update(lut_num, panel_info.epdc_data.wv_modes.mode_gc16,
		...
		// can continue with quickboot as soon as fb has been copied
		for (i = 0; i < 40; i++) {
			if (!epdc_is_wb_active()) {
				debug("Continuing with quickboot\n");
				return;
			}
			msleep(10); // this is much faster than LUT so wake more quickly
		}
		debug("Splash screen update failed!\n");
	}
}
...
#endif
/*
 * If we saved the actual working buffer to disk, just load it
 *
 * Otherwise, clear the screen to white with a mode0 update, and set the working
 * buffer to reflect that every pixel is currently white.
 */
void make_working_buffer_sane(void) {
	static int is_buffer_sane = 0;
...
void lcd_enable(void)
{
	...
	make_working_buffer_sane();
        ...
        if (gd->flags & GD_FLG_CRIT_BATT) {
                blit_crit_batt_img(lcd_base, panel_info.vl_col, panel_info.vl_row, &col, &row, &cols, &rows);
        } else {
                blit_splash_img(lcd_base, panel_info.vl_col, panel_info.vl_row, &col, &row, &cols, &rows);
        }

	cache_flush();

	draw_splash_screen(col, row, cols, rows);

	// leave these on during quickboot while splash screen continues updating
        if (!(gd->flags & (GD_FLG_QUICKBOOT | GD_FLG_CRIT_BATT))) {
		lcd_disable();
		lcd_panel_disable();
	}
}
