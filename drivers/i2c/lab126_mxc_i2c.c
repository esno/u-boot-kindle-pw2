...
static struct mxc_i2c_base i2c_base[] = {
...
{
	static unsigned int last_bus = -1;
...
int i2c_write(uchar chip, uint addr, int alen, uchar *buf, int len)
{
	...
	while (i2c_dev.dlen--)
		if (i2c_tx_byte(*buf++)) {
			printf("%s transmit failed at %d\n", __FUNCTION__, i2c_dev.dlen);
			return -1;
        	}
...
int i2c_imx_bus_busy(int for_busy)
{
	...
	while (--timeout) {
		temp = __REG16(i2c_dev.i2c_comm->base_i2sr);
...
/**
 * Follow linux i2c sequence to remove the extra byte at the end of the read.
 * 
 */
...
static int i2c_imx_start(void)
{
	...
	/* Start I2C transaction */
        __REG16(i2c_dev.i2c_comm->base_i2cr) = I2CR_IEN |  I2CR_MSTA;

        if (i2c_imx_bus_busy(1) == 0) {
                printf("%s 1- i2c_imx_bus_busy(1) failed\n", __func__);
                return -1;
        }

        __REG16(i2c_dev.i2c_comm->base_i2cr) = I2CR_IEN | I2CR_IIEN | I2CR_MSTA | I2CR_MTX;
	return 0;
}

/*
 * Follow the correct sequence in linux driver.
 * Now it does not have an extra byte read at the end of i2c_read and
 * does not need to have delay between the bytes.
 *
 */
int i2c_raw_read(uchar chip, uchar *buf, int len)
{
	...
	DPRINTF("%s chip: 0x%02x len: %d\n",__FUNCTION__, chip, len);

	i2c_imx_start();

	if (i2c_tx_byte(chip << 1 | 1)) {
		/* Disable I2C controller */
		__REG16(i2c_dev.i2c_comm->base_i2cr) = 0;
		return -1;
	}

	__REG16(i2c_dev.i2c_comm->base_i2cr) = I2CR_IEN | I2CR_IIEN |  I2CR_MSTA | ((len == 1) ? I2CR_TX_NO_AK : 0);
	/* Dummy read */
	ret = __REG16(i2c_dev.i2c_comm->base_i2dr);
	while (len--) {
		if (i2c_wait_busy() == 0) {
		...
		if (len == 1)
			__REG16(i2c_dev.i2c_comm->base_i2cr) = I2CR_IEN | I2CR_IIEN | I2CR_MSTA | I2CR_TX_NO_AK;
		if (len == 0)
			__REG16(i2c_dev.i2c_comm->base_i2cr) = I2CR_IEN | I2CR_IIEN | I2CR_TX_NO_AK;
		ret = __REG16(i2c_dev.i2c_comm->base_i2dr);
		*buf++ = ret;
	}
