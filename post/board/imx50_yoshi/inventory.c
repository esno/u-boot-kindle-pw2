...
int inventory_post_test (int flags)
{
    ...
    else if (BOARD_IS_ICEWINE(rev))  // Only on Icewine WAN boards
    {
        index = INDEX_ICEWINE;
        ret  = check_touch(index);    // Cypress are not preprogrammed
        ret  = check_proximity();     // Proximity may not be on board yet for pre evt
        ret  = check_battery();       // Battery may not be installed at SMT station
        ret |= check_als();
        ret |= check_power();
        ret |= check_papyrus(index);  
    ...
    else if (BOARD_IS_ICEWINE_WFO(rev))  // Only on Icewine WFO boards
    {
        index = INDEX_ICEWINE;
        ret  = check_touch(index);  // Cypress are not preprogrammed
        ret  = check_proximity();   // Proximity may not be on board yet for pre evt
